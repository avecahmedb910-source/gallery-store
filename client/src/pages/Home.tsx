    ...
    <div dir="rtl" className="min-h-screen bg-[var(--brand-surface)] text-[var(--brand-ink)] pb-20">
      ...
      <main>
        ...
        <section id="products" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-24">
          ...
          {isLoading ? <div className="rounded-3xl bg-white p-12 text-center font-bold text-[#927b71]">بنجهز المنتجات ليكي...</div> : products.length === 0 ? <div className="rounded-3xl border border-dashed border-[#d8c4b5] bg-white p-12 text-center"><Sparkles className="mx-auto text-[#c58b65]"/><h3 className="mt-4 text-xl font-black">الكتالوج بيتجهز</h3><p className="mt-2 text-sm text-[#927b71]">أضيفي أول منتجاتك من لوحة الإدارة عشان تظهر هنا.</p><Link href="/admin"><Button className="mt-6 rounded-full bg-[var(--brand-plum)] font-black">دخول لوحة الإدارة</Button></Link></div> : <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{products.map((product) => <Link key={product.id} href={`/product/${product.slug}`} className="group overflow-hidden rounded-[2rem] border border-[#eadfd3] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="relative aspect-[4/3] overflow-hidden bg-[#f3ebe4]">{product.images[0] && <img src={product.images[0]} alt={product.name} onError={(event) => { event.currentTarget.src = "/manus-storage/gallery-store-logo-hd_698a1ec8.png"; }} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/>}{product.compareAtPrice && product.compareAtPrice > product.price && <span className="absolute right-4 top-4 rounded-full bg-[var(--brand-rose)] px-3 py-1 text-xs font-black text-white">خصم {Math.round((1 - product.price / product.compareAtPrice) * 100)}%</span>}</div><div className="p-5"><h3 className="text-lg font-black">{product.name}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--brand-muted)]">{product.description}</p><div className="mt-5 flex items-center justify-between"><div><span className="text-xl font-black text-[var(--brand-rose)]">{formatPrice(product.price)}</span>{product.compareAtPrice && <del className="mr-2 text-xs text-[#aa9b94]">{formatPrice(product.compareAtPrice)}</del>}</div><span className="flex items-center gap-1 text-xs font-black text-[var(--brand-plum)]">التفاصيل <ChevronLeft size={16}/></span></div></div></Link>)}</div>}
        </section>
