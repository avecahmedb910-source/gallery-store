...
export async function getProductBySlug(slug: string) {
  ...
  return rows[0] ? serializeProduct(rows[0]) : undefined;
}
...
export async function createProduct(input: Omit<InsertProduct, "images" | "variants"> & { images: string[]; variants: string[] }) {
  const db = await getDb();
...
export async function updateProduct(id: number, input: Partial<Omit<InsertProduct, "images" | "variants">> & { images?: string[]; variants?: string[] }) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
